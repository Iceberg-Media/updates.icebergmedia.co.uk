...
The header and footer navigation use this same machinery for you. You write each link once in `nav.config.ts`; on every locale the Header and Footer prefix its path (`/blog` → `/nl/blog`, leaving external and `#anchor` links alone), point the logo at the locale's home, and translate its label when the item has a `labelKey` (the bundled items use `nav.items.*`). A locale needing a different slug or label can set a per-locale `locales` override on the item — but for most sites, translating the `nav.items.*` strings is all it takes.
